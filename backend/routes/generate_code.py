...
class ExtractedParams:
    ...
    # OpenAI-compatible gateways (OpenRouter, NVIDIA, Kilo, Zen), keyed by
    # gateway id. Values are the Settings-dialog overrides only; the `.env`
    # fallback lives in llm_gateways.Gateway.resolve_api_key so the env var
    # names stay declared in exactly one place.
    gateway_api_keys: Dict[str, str] = field(default_factory=dict[str, str])
    gateway_base_urls: Dict[str, str] = field(default_factory=dict[str, str])
...
class ParameterExtractionStage:
    ...
    async def extract_and_validate(self, params: Dict[str, Any]) -> ExtractedParams:
        ...
        # OpenAI-compatible gateways: the key is what unlocks a provider, the
        # base URL is for proxies / self-hosted endpoints.
        gateway_values = gateway_settings_values(params)
        gateway_api_keys: Dict[str, str] = {}
        gateway_base_urls: Dict[str, str] = {}
        for gateway_id, gateway in GATEWAYS.items():
            api_key = gateway_values.get(gateway.settings_key)
            if api_key:
                print(f"Using {gateway.settings_key} from client-side settings dialog")
                gateway_api_keys[gateway_id] = api_key
            # In prod, user-supplied base URLs are ignored (see the factory),
            # same as the OPENAI_BASE_URL override above.
            if not IS_PROD:
                base_url = gateway_values.get(gateway.settings_base_url_key)
                if base_url:
                    gateway_base_urls[gateway_id] = base_url
        ...
        return ExtractedParams(
            ...
            gateway_base_urls=gateway_base_urls,
        )
...
class NoUsableModelError(Exception):
    """Model selection failed for a reason the user can act on.

    The message is safe to show verbatim, so it bypasses the generic
    "add an API key" fallback that swallows every other selection error.
    """
...
class ModelSelectionStage:
    ...
    async def select_models(
        ...
        gateway_api_keys: Dict[str, str] | None = None,
    ) -> List[Llm]:
        ...
        try:
            ...
            await self.throw_error(
                "No model provider key found. Please add the environment variable "
                "OPENAI_API_KEY, ANTHROPIC_API_KEY, GEMINI_API_KEY, or one of the "
                "OpenAI-compatible gateway keys (OPENROUTER_API_KEY, NVIDIA_API_KEY, "
                "KILO_API_KEY, ZEN_API_KEY) to backend/.env or in the settings dialog. "
    ...
    def _gateway_models(
        ...
        input_mode: InputMode,
        num_variants: int,
        gateway_api_keys: Dict[str, str] | None,
    ) -> List[Llm]:
        """Models from every gateway that has a key, best-supported first.

        Image and video requests need a model that can actually see the
        screenshot; plenty of free-tier slugs are text-only and would happily
        invent a layout from the prompt alone.
        """
        gateways = configured_gateways(gateway_api_keys)
        if not gateways:
            raise Exception("No OpenAI, Anthropic, Gemini, or gateway key")

        vision_required = input_mode in ("image", "video")
        ...
        if not models:
            names = ", ".join(gateway.display_name for gateway in gateways)
            raise NoUsableModelError(
                f"None of the models {names} offers accept image input, which a "
                "screenshot needs. Use a text prompt instead, or add a "
                "vision-capable model to that gateway's list in "
                "backend/llm_gateways.py."
            )
        print("Using gateway models: " + ", ".join(model.value for model in models))
        return models
    ...
    def _get_variant_models(
        ...
        if gemini_api_key and anthropic_api_key and openai_api_key:
            ...
            # No frontier key: use the OpenAI-compatible gateways instead, so a
            # free or low-cost key can still drive a generation. Frontier keys
            # always win above, because judged quality on gateways is lower.
            models = self._gateway_models(input_mode, num_variants, gateway_api_keys)
...
class AgenticGenerationStage:
    ...
    def __init__(
        ...
        gateway_api_keys: Dict[str, str] | None = None,
        gateway_base_urls: Dict[str, str] | None = None,
    ):
        ...
        self.gateway_api_keys = gateway_api_keys or {}
        self.gateway_base_urls = gateway_base_urls or {}
        self.generation_id = (
            generation_id
    ...
    async def _run_variant(
        ...
        try:
            ...
            runner = Agent(
                ...
                gateway_base_urls=self.gateway_base_urls,
            )
    ...
    async def process(
        ...
        try:
            ...
            context.variant_models = await model_selector.select_models(
                ...
                gateway_api_keys=context.extracted_params.gateway_api_keys,
            )
            if IS_DEBUG_ENABLED:
            ...
            generation_stage = AgenticGenerationStage(
                ...
                generation_type=context.extracted_params.generation_type,
                gateway_api_keys=context.extracted_params.gateway_api_keys,
                gateway_base_urls=context.extracted_params.gateway_base_urls,
            )
