...
- OpenAI-compatible gateways (OpenRouter, NVIDIA NIM, Kilo, OpenCode Zen, ZenMux) are registered in `backend/llm_gateways.py`: base URL, env var names, model ids, and per-model `supports_vision`/`supports_tools` flags all live there, while `backend/llm.py` holds the matching `Llm` members (value = `<gateway>/<model id>`). Add a gateway model in both places; `test_llm_gateways.py` fails if they drift. Gateway models talk Chat Completions (`agent/providers/openai_compatible.py`), not the Responses API the first-party OpenAI session uses.
