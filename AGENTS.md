...
- Core feature (screenshot → code) requires at least one LLM key: set `OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, `GEMINI_API_KEY`, or a gateway key (`OPENROUTER_API_KEY`, `NVIDIA_API_KEY`, `KILO_API_KEY`, `ZEN_API_KEY`, `ZENMUX_API_KEY`) in `backend/.env` (restart backend after editing) or in the in-app Settings dialog. Without a key, generation fails fast with a "No model provider key found" message.
- OpenAI-compatible gateways (OpenRouter, NVIDIA NIM, Kilo, OpenCode Zen, ZenMux) are registered in `backend/llm_gateways.py`: base URL, env var names, model ids, and per-model `supports_vision`/`supports_tools` flags all live there, while `backend/llm.py` holds the matching `Llm` members (value = `<gateway>/<model id>`). Add a gateway model in both places; `test_llm_gateways.py` fails if they drift. `frontend/src/lib/gateways.ts` mirrors the same model list (badges are keyed off the slug: a `:free`, `-free`, or `/free` id must be labelled `Free` in `lib/models.ts`, which `lib/gateways.test.ts` enforces). Gateway models talk Chat Completions (`agent/providers/openai_compatible.py`), not the Responses API the first-party OpenAI session uses.
