...
No OpenAI, Anthropic, or Gemini key? The app also runs on any
**OpenAI-compatible gateway** - OpenRouter, NVIDIA NIM, Kilo, and OpenCode Zen
are built in, and their `:free` model slugs cost nothing. See
[Free and low-cost providers](#-free-and-low-cost-providers) below.
...
# Optional: free / low-cost OpenAI-compatible gateways (see the FAQ below).
...
You can also set up OpenAI, Anthropic, Gemini, and gateway keys using the settings dialog in the frontend (click the gear icon after loading the app). Replicate must be configured in `backend/.env` as `REPLICATE_API_KEY`. The Settings dialog also shows whether **screenshot preview** is available on your backend.
...
OpenRouter, NVIDIA NIM, Kilo, and OpenCode Zen all expose an OpenAI-compatible
`/chat/completions` endpoint, so the backend talks to them with the OpenAI
...
- Add the key in the Settings dialog or `backend/.env`; a `.env` change needs a
  backend restart.
- Gateway models are the **fallback**: with an OpenAI, Anthropic, or Gemini key
  present, those models are used instead. This is deliberate — see the next
  point.
- **Expect worse output.** Screenshot-to-code is a hard task (vision + a long,
  structured HTML answer), and free-tier models are noticeably worse at it than
  GPT, Claude, or Gemini. The UI flags those variants with a green **Free**
  badge so you can compare rather than assume.
- **Vision is mandatory for image and video input.** Many `:free` slugs are
  text-only, and a text-only model will happily describe a page it never saw.
  Each gateway model is marked `supports_vision` in `llm_gateways.py`, and
