...
from llm_gateways import gateway_for_model, supports_vision
...
async def _run_eval_agent(
    ...
    # Gateway models resolve their key from `.env` inside the provider factory;
    # this only turns a missing key into a named, per-eval failure.
    gateway = gateway_for_model(model)
    if gateway is not None and not gateway.resolve_api_key():
        raise Exception(f"{gateway.display_name} API key not found")
    if input_mode == "image" and not supports_vision(model):
        raise Exception(
            f"{model.value} cannot read the screenshot an eval runs on; it only "
            "accepts text input."
        )
