...
# Variant models used when a first-party key is present. The sets are ordered by
# how much each take is worth its cost: the first entry anchors quality, the
# last is the fast lane.
#
# Runs with *only* an OpenAI-compatible gateway key (OpenRouter, NVIDIA, Kilo,
# Zen) are not listed here: their variants are built at request time from
# llm_gateways.GATEWAYS, so adding a gateway model does not mean editing a
# second table.
