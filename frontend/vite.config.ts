...
export default ({ mode }) => {
  ...
  return defineConfig({
    ...
    server: {
      ...
      host: true,
      // Vite rejects requests whose Host header it doesn't recognize, which
      // breaks remote dev (containers, tunnels, sandbox previews). This is a
      // dev-only server with no auth of its own, so accept any host by default;
      // set VITE_ALLOWED_HOSTS=a.example.com,b.example.com to pin it instead.
